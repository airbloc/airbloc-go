...
func (client *Client) Create(walletAddress ethCommon.Address, password string) (*accounts.Session, error) {
...
func (client *Client) LogIn(identity string, password string) (*accounts.Session, error) {
	...
	session := accounts.NewSession(accountId, ethCommon.HexToAddress(response.GetOwnerAddress()), password)
