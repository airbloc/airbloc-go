...
func (mt *MainTree) Leaves() map[types.ID][]types.RowId {
	leaves := make(map[types.ID][]types.RowId, len(mt.leaves))
...
func (mt *MainTree) GenerateProof(rowId types.RowId, userId types.ID) ([]byte, error) {
...
func verifyMainProof(userId types.ID, subRoot, mainRoot, proofBits, proofBytes []byte) bool {
	k := userId.Uint64()
...
func VerifyMainProof(rowId types.RowId, userId types.ID, mainRoot, proof []byte) bool {
	// split proofs
	// main
	proofBits, proof := proof[:types.IDLength], proof[types.IDLength:]
	count := 0
	for i := 0; i < len(proofBits)*8; i++ {
		if hasBit(proofBits, uint64(i)) {
			count++
...
func NewMainTree(input map[types.ID][]types.RowId) (*MainTree, error) {
	// check input
