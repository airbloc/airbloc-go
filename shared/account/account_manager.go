...
func (manager *Manager) CreateTemporary(identityHash ethCommon.Hash) (types.ID, error) {
	...
	accountId := types.ID(event.AccountId)
	manager.log.Info("New temporary account created.", logger.Attrs{"id": accountId.Hex()})

	return accountId, nil
}

func (manager *Manager) CreateUsingProxy(owner ethCommon.Address, passwordSignature []byte) (types.ID, error) {
	...
	return types.ID(event.AccountId), err
}

func (manager *Manager) Get(accountId types.ID) (*Account, error) {
...
func (manager *Manager) IsDelegateOf(delegateAddr ethCommon.Address, accountId types.ID) (bool, error) {
